...
定义小程序首次用户链路的统一状态推导、API base URL 配置、跨页面串联和降级边界，确保新用户从登录、创建隐者档案、配置工作档案到返回首页查看已摸金额的流程基于真实状态推进，不依赖演示数据或独立完成标记。
...
- **AND** 首页 SHALL 仅展示必要状态、占位金额/倒计时和配置入口，不再展示“填写后首页会显示”等重复说明
