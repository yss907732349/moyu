...
系统 SHALL 对社区评论和回复执行三通道审核分流，并 SHALL 将每次审核决策归一化为 `auto_approve`、`auto_reject` 或 `manual_review`。
...
- **WHEN** 评论或回复命中灰区风险、上下文不明、用户风险较高、频率异常或审核服务不可确认
- **THEN** 系统 SHALL 将审核决策设为 `manual_review`
- **AND** 系统 SHALL 创建状态为 `pending` 的评论或回复
- **AND** 公开评论区 SHALL NOT 展示该内容直到人工复核通过

#### Scenario: 待复核评论回复作者自己可见

- **WHEN** 评论或回复未被明确驳回但进入人工复核
- **THEN** 提交者本人查看帖子详情时 SHALL 看到该评论或回复
- **AND** 响应 SHALL 标记该内容仅作者自己可见
- **AND** 其他用户 SHALL NOT 在帖子详情或公开评论区看到该内容
- **AND** 人工复核通过后该内容 SHALL 按公开评论或回复展示

### Requirement: 可信用户快速通过门禁

系统 SHALL 使用用户风险、内容风险和提交行为共同决定评论或回复是否可以快速通过。

#### Scenario: 用户满足可信门槛

- **WHEN** 用户已登录、已创建隐者档案、已通过冷启动期且近期无驳回、有效举报、禁言、封禁或 `limited` 治理状态
- **AND** 用户提交低风险短评论或短回复
- **THEN** 系统 SHALL 允许该内容进入快速通过判断

#### Scenario: 冷启动用户不直接快速通过

- **WHEN** 新用户或未通过冷启动期的用户提交评论或回复
- **AND** 文本未命中明确违规风险
- **THEN** 系统 SHALL 将该内容设为 `pending` 或按更严格策略处理
- **AND** 系统 SHALL NOT 仅因文本低风险就直接公开
...
#### Scenario: 验证可信低风险评论快速通过

- **WHEN** 开发者运行评论审核减负验证命令
- **THEN** 验证 SHALL 构造可信用户提交低风险短评论的场景
- **AND** 验证 SHALL 断言评论状态为 `approved` 且审核决策为 `auto_approve`

#### Scenario: 验证风险用户进入人工复核

- **WHEN** 开发者运行评论审核减负验证命令
- **THEN** 验证 SHALL 构造新用户、近期违规用户或 `limited` 用户提交低风险评论的场景
